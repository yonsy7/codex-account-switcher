...
from codex_switcher.keychain import read_credentials, write_credentials, delete_credentials

FAKE_CREDS = json.dumps({"accessToken": "sk-ant-oat01-xxx", "refreshToken": "sk-ant-ort01-xxx"})


class TestReadCredentials:
    @patch("codex_switcher.keychain.subprocess.run")
    def test_read_existing_entry(self, mock_run):
        mock_run.return_value = MagicMock(returncode=0, stdout=FAKE_CREDS)
        result = read_credentials("codex-switcher:emile@gmail.com")
        assert result == FAKE_CREDS
        mock_run.assert_called_once_with(
            ["security", "find-generic-password", "-s", "codex-switcher:emile@gmail.com", "-w"],
            capture_output=True,
            text=True,
        )
    ...
    def test_read_missing_entry_returns_none(self, mock_run):
        ...
        result = read_credentials("codex-switcher:missing@test.com")
...
class TestWriteCredentials:
    ...
    def test_write_credentials_deletes_then_adds(self, mock_run):
        ...
        write_credentials("codex-switcher:emile@gmail.com", "emilejouannet", FAKE_CREDS)
        assert mock_run.call_count == 3
        # Verify delete calls
        mock_run.assert_any_call(
            ["security", "delete-generic-password", "-s", "codex-switcher:emile@gmail.com"],
        ...
        mock_run.assert_any_call(
            [
                ...
                "-s", "codex-switcher:emile@gmail.com",
                "-a", "emilejouannet",
                "-w", FAKE_CREDS,
            ],
            capture_output=True,
    ...
    def test_write_no_existing_entry(self, mock_run):
        ...
        write_credentials("codex-switcher:new@test.com", "newuser", FAKE_CREDS)
    ...
    def test_write_failure_raises(self, mock_run):
        ...
        try:
            write_credentials("codex-switcher:test@test.com", "testuser", FAKE_CREDS)
...
class TestDeleteCredentials:
    ...
    def test_delete_existing(self, mock_run):
        ...
        result = delete_credentials("codex-switcher:emile@gmail.com")
        assert result is True

    @patch("codex_switcher.keychain.subprocess.run")
    def test_delete_missing_returns_false(self, mock_run):
        mock_run.return_value = MagicMock(returncode=44)
        result = delete_credentials("codex-switcher:missing@test.com")
...
class TestReadAccountAttribute:
    ...
    def test_read_account_attribute(self, mock_run):
        ...
        from codex_switcher.keychain import read_account_attribute
        result = read_account_attribute("Codex-credentials")
